...
FILE STRUCTURE OPTION 1 — single diagram
...
FILE STRUCTURE OPTION 2 — multiple diagrams
...
AUTHORING TIPS
  Use an invisible link (A ~~~ B) when B should appear after A without a visible edge.
  Use invisible links often between subgraph identifiers to control the order of disconnected subgraphs.

  Step sequencing:
    1. Overview step — no highlights, lets the viewer see the full diagram
    2. Entry point — first action
  ...
  When to use multiple diagrams:
    - If you are explaining different concerns. E.g. conceptual overview, system diagram, business logic, and the state machine should all go in separate sections
  ...
  Where should the ariel.yaml file live:
    ...
    (e.g. generating SVGs for PRs or brief explanations), save the
